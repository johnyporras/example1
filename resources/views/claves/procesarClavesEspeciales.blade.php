@extends('layouts.app')
@section('title','Autorizar Claves Especiales')
@section('content') 
{!! Form::open(['url' => 'clavesEspeciales/rechazar', 'class' => 'form-horizontal', 'name' => 'procesarClavesEspeciales',
                                                       'id'=>'procesarClavesEspeciales', 'method' => 'aprobar' ]) !!}
 <h4>Paciente</h4>
@if (isset($data_clave))
        ...
        <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ $data->motivo}}</td>              
                     <td><a href="consultarDocumento?clave={{$data->clave}}">{{ $data->documentos}}</a></td>
                </tr>
            </tbody>
        </table>
    </div>     
  @endforeach 
   ...
   <div class="form-group">    
      <div class="col-sm-offset-2 col-sm-3">  
          <p><a href="{{ url('clavesEspeciales/aprobar?id_afiliado_temporal='.$data->id_afiliado_temporal.'&id_clave='.$data->id_clave) }}"  class="btn btn-primary form-control ">Aprobar</a>
          </p>    
      </div>
        <div class="col-sm-offset-2 col-sm-3"><!--   -->
...
<script>
 $(document).ready(function() {
     $("#procesarClavesEspeciales").submit(function(e){
         if ( $("#rechazo").val() == ''){
